...
import { Maximize2, ExternalLink, HardDrive, Info, Sparkles, Play, Film } from "lucide-react";
...
export default function VideoPlayer({
  ...
  const embedUrl = details?.embedUrl || getDriveEmbedUrl(driveFileId);
    ...
    <div className="relative w-full rounded-2xl overflow-hidden bg-black shadow-lg border border-[#181822]">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span>
            ...
            <strong className="text-zinc-200">
              {isYouTube ? "YouTube Stream" : isDirect ? "Cloud Media Player" : "Google Drive Free Storage"}
            </strong>
          </span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
          <span className="text-[10px] text-zinc-400">Zero-Cost CDN</span>
        </div>
      </div>
      ...
      {hasError && (
        <div className="p-4 bg-rose-500/10 border-t border-rose-500/30 text-xs text-rose-300 flex items-center gap-3">
          ...
          <div>
            Video not displaying? Ensure the video link is public or shared as:{" "}
